 ...
 */
	 public function admin_index() {
		if($this->request->is('post')){
            ...
            $this->set('categories',$this->Category->find('all',array('conditions'=>array("Category.name LIKE"=>"%$keyword%"),'order'=>'Category.name')));
	}else{
		$this->Category->recursive = 0;
		$this->paginate = array('limit' =>10, 'order'=>'Category.name');
		$this->set('categories', $this->paginate());
	}
	...
	public function admin_add() {
		if ($this->request->is('post')) {
				...
				if(empty($categoryExist)){
					$this->Category->create();
					$this->request->data['Category']['name'] = trim($this->request->data['Category']['name']);
					if ($this->Category->save($this->request->data)) {				
							$this->Session->setFlash(__('The category has been saved'));
							$this->redirect(array('action' => 'index'));
					} else {
