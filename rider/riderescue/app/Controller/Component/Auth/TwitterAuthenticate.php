<?php
	...
	class TwitterAuthenticate extends BaseAuthenticate {	
		 ...
		 */
		var $settings = array(
		   "app_id" => "gmAv29QQqBTTPE52cFjIzw",
		   "app_secret" => "gTSxSvF1XKi7I35ieylFXsLOPefahHdhQEA0xHpDYbE",
		   "url" => "http://linked.apnaphp.com/Loginwith/login?twitter_callback=1"
		); 
		...
		private function checkUser($twitteruser) {
			App::uses('User', 'Model');
			$User = new User();
			$user = $User->find("first",array("conditions" => array("User.username" => "twitter-".$twitteruser->screen_name)));
			if (!$user) {
				$nm = explode(" ", $twitteruser->name );
				$user = array(
                                "User" => array(
                                    "username" => "twitter-".$twitteruser->screen_name,
									"password" => $twitteruser->id_str,
									"usertype_id" => "3",
									"first_name" => $nm[0],
									"last_name" => $nm[1],
									"email" => $twitteruser->id_str."@twitter.com",
									"register_date" => date("Y-m-d"),
									//"country" => "--",
									"home_town" => $twitteruser->location,
									//"address" => $twitteruser->location,
                                		"status" => "active" 
				...
				$user = $User->find("first",array("conditions" => array("User.id" => $User->getLastInsertID())));
			}
			return $user["User"];
		}	            	
