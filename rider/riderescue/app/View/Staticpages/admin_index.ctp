...
<?php $i=1; ?>
			...
			<td> <?php  echo (strlen(strip_tags($user['Staticpage']['description'])) > 30? substr(strip_tags($user['Staticpage']['description']),0,30).'...' : strip_tags($user['Staticpage']['description'])); ?> </td>						
            <td><?php echo date("d M y",strtotime($user['Staticpage']['created'])); ?></td>
             <td class="center">
             <form></form>
			<?php echo $this->Html->link('<span class="edit-icn"></span>',array('action' => 'admin_edit', $user['Staticpage']['id']),array('class'=>'tablectrl_small bDefault tipS tool-tip','title'=>'Edit','escape'=>false)); ?>
            </td>
            </tr>
		<?php $i++;?>	
            <?php endforeach; ?>
