...
<div id="content">
    ...
    <div class="wrapper">
     ...
     <div class="nNote nSuccess" id="flash">
       <div class="alert alert-success" style="text-align:center" ><?php echo $x; ?></div>
                   ...
                   <ul class="middleNavA">
            <li><a href="<?php echo $this->Html->url(array('controller'=>'Blogs','action'=>'index')); ?>" title="Add Blog"  class="tool-tip"><?php echo $this->Html->image('/images/icons/color/order-149.png'); ?><span>Add Blog</span></a></li>
        </ul> 
      <!--  <ul class="middleNavR">
	...
	<!------------------end--------------------------->
       ...
       <div class="widget check grid6">
        ...
        <!--</div></span>--><h6>Blog Management</h6>
...
<th><?php echo('Created'); ?></th>
            ...
            <tr class="gradeX" id="order_<?php echo $data['Blog']['id']; ?>">
             <td><?php echo $this->Form->checkbox("use"+$data['Blog']['id'],array('value' => $data['Blog']['id'],'class'=>'checkAll')); ?></td>
             <td><?php echo h($data['Blog']['title']); ?></td>
            <td><?php echo (strlen(strip_tags($data['Blog']['content']))>30 ? substr(strip_tags($data['Blog']['content']),0,30).'...' : strip_tags($data['Blog']['content'])); ?></td>
            
			
            <td><img src="<?php echo $this->webroot;?>blogs/<?php echo h($data['Blog']['image']);?>" alt="<?php echo h($data['Blog']['image']); ?>" style="width:140px; height:50px;"></td>
           <td><?php echo h($data['Blog']['link']); ?></td>
		   <td><?php if($data['Blog']['status'] == '0'){echo "Deactivated";}else if($data['Blog']['status'] == '1'){echo "Activated";} ?></td>
		   <td><?php echo date("d-M-Y",strtotime($data['Blog']['created'])); ?></td>
            <td class="center">
             <form></form>
            <a href="<?php echo $this->Html->url(array('action' => 'edit', $data['Blog']['id'])); ?>" class="tablectrl_small bDefault tipS tool-tip" title="Edit"  style="margin-bottom:-7px;"><span class="edit-icn" data-icon="&#xe1db;"></span></a>
             <?php echo $this->Form->postLink('<span class="iconb" data-icon="&#xe136;"></span>',array('controller'=>'Blogs','action'=>'delete',$data['Blog']['id']),array('escape'=>false,'class'=>'tablectrl_small bDefault tipS tool-tip','title'=>'Delete'),__('Are you sure to want to delete?'));?>    
                          <?php if ($data['Blog']['status']=='0'){ ?>

			<?php echo $this->Form->postLink('<span class="iconb" data-icon="&#xe1bf;"></span>', array('action' => 'activate', $data['Blog']['id']),array('escape'=>false,'class'=>'tablectrl_small bDefault tipS tool-tip','title'=>'Active'));?><?php }else { ?>

 <?php echo $this->Form->postLink('<span class="iconb" data-icon="&#xe1c1;"></span>', array('action' => 'block', $data['Blog']['id']), array('escape'=>false,'class'=>'tablectrl_small bDefault tipS tool-tip','title'=>'Deactive')); ?><?php }?>
